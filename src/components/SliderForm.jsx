  ...
  return (
    <div>
      {slides.map((slide, index) => (
        <div key={index} className="flex z-50 sm:flex-row mx-auto justify-center   flex-col gap-4 items-center">
          <h3 className="font-bold sm:text-xl">Slider {index + 1}</h3>
          <input
            type="text"
            placeholder="Background Image"
            value={slide.backgroundImage}
            className=" w-48 sm:w-72 rounded h-10 bg-red-500"
            onChange={(e) =>
          ...
          <button
            className="p-2 px-4 m-1 bg-black text-white font-bold text-lg rounded hover:bg-gray-200  hover:text-black transition-all"
